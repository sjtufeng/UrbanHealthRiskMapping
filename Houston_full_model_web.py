...
# %%
filename_Houston_311_agg_tract = os.path.join(dirname, 'machine_learning/ml_Houston_311_agg_tract.csv')
df_Houston_311_agg_tract = pd.read_csv(filename_Houston_311_agg_tract, dtype={'tract2010': str})
# %%
ser_count311 = df_Houston_311_agg_tract.drop(columns=['tract2010']).sum()
...
c2 = ['Water Leak',
      ...
      'Water Service',
      'Drainage']
c3 = ['Traffic Signal Maintenance',
      'Traffic Signs']
c4 = ['Street Condition', 'Street Hazard']
c5 = ['Parking Violation']
c6 = ['Building or Sign Code']
c7 = ['Nuisance On Property']
c8 = ['PWE CIP Project']

df_Houston_311_agg_tract['agg_ALL'] = df_Houston_311_agg_tract.loc[:, all_311_types].sum(axis=1)
df_Houston_311_agg_tract['agg_trash'] = df_Houston_311_agg_tract.loc[:, c1].sum(axis=1)
df_Houston_311_agg_tract['agg_water'] = df_Houston_311_agg_tract.loc[:, c2].sum(axis=1)
df_Houston_311_agg_tract['agg_traffic_sign'] = df_Houston_311_agg_tract.loc[:, c3].sum(axis=1)
df_Houston_311_agg_tract['agg_street'] = df_Houston_311_agg_tract.loc[:, c4].sum(axis=1)
df_Houston_311_agg_tract['agg_parking'] = df_Houston_311_agg_tract.loc[:, c5].sum(axis=1)
df_Houston_311_agg_tract['agg_building_sign'] = df_Houston_311_agg_tract.loc[:, c6].sum(axis=1)
df_Houston_311_agg_tract['agg_nuisance_property'] = df_Houston_311_agg_tract.loc[:, c7].sum(axis=1)
df_Houston_311_agg_tract['agg_PWE_CIP'] = df_Houston_311_agg_tract.loc[:, c8].sum(axis=1)

df_Houston_311 = pd.concat([df_Houston_311_agg_tract[['tract2010']], df_Houston_311_agg_tract.loc[:, 'agg_ALL':]],
                           axis=1)

# %%
filename_Houston_SLD_agg_tract = os.path.join(dirname, 'machine_learning/ml_Houston_SLD_agg_tract.csv')
df_Houston_SLD_agg_tract = pd.read_csv(filename_Houston_SLD_agg_tract, dtype={'tract_id': str})
# %%
cols_sld = ['tract_id', 'P_WRKAGE',
            ...
            'D4b025', 'D4b050', 'D4d']
df_Houston_SLD_agg_tract = df_Houston_SLD_agg_tract[cols_sld]
# %%
filename_Houston_SVI = os.path.join(dirname, 'machine_learning/ml_Houston_SVI.csv')
df_Houston_SVI = pd.read_csv(filename_Houston_SVI, dtype={'FIPS': str})
# %%
cols_svi = ['FIPS'] + [c for c in df_Houston_SVI.columns if c.startswith('EP_')]
df_Houston_SVI = df_Houston_SVI[cols_svi]
# %%
filename_Houston_CDC = os.path.join(dirname, 'machine_learning/ml_Houston_CDC.csv')
df_Houston_CDC = pd.read_csv(filename_Houston_CDC, dtype={'TractFIPS': str})
# %%
...
# %%
# merge data
d1 = pd.merge(df_Houston_CDC[['TractFIPS', 'Population2010'] + health_vars], df_Houston_311, left_on='TractFIPS',
              right_on='tract2010', how='left').drop(columns='tract2010')
d2 = pd.merge(d1, df_Houston_SLD_agg_tract, left_on='TractFIPS', right_on='tract_id', how='left').drop(
    columns='tract_id')
d3 = pd.merge(d2, df_Houston_SVI, left_on='TractFIPS', right_on='FIPS', how='left').drop(columns='FIPS')
df_Houston_all_data = d3
# %%
# adjust the 311 counts by population
df_Houston_all_data_adj = df_Houston_all_data.copy()
cols_311_types = df_Houston_311.columns.to_list()
cols_311_types.remove('tract2010')
df_Houston_all_data_adj[cols_311_types] = df_Houston_all_data_adj[cols_311_types].div(
    df_Houston_all_data_adj['Population2010'], axis=0)
# %%
# check null values
cols_nan = df_Houston_all_data_adj.columns[(df_Houston_all_data_adj.isnull().sum() != 0)].to_list()
df_Houston_all_data_adj[cols_nan].isnull().sum()
# %%
df_Houston_all_data_adj.dropna(subset=cols_nan, inplace=True)
# %%
df_Houston_tracts = df_Houston_all_data_adj[['TractFIPS']].copy()
df_Houston_pop = df_Houston_all_data_adj[['Population2010']].copy()
df_Houston_all_data_adj.drop(columns=['Population2010', 'TractFIPS'], inplace=True)

# %%
# shorten column names (<= 10 characters)
col_name_map = {'agg_ALL': 'SR_ALL', 'agg_trash': 'SR_TRASH', 'agg_water': 'SR_WATER', 'agg_traffic_sign': 'SR_TRFSIGN',
                'agg_street': 'SR_STREET', 'agg_parking': 'SR_PRKNG', 'agg_building_sign': 'SR_BLDSIGN',
                'agg_nuisance_property': 'SR_NUISANC', 'agg_PWE_CIP': 'SR_PWECIP',
...
df_Houston_all_data_adj.rename(columns=col_name_map, inplace=True)
...
# %%
train_set, test_set = train_test_split(df_Houston_all_data_adj, test_size=0.2, random_state=42)
...
df_Houston_map_actual = pd.concat([df_Houston_tracts, df_Houston_pop, df_Houston_all_data_adj], axis=1).loc[my_indices,
                        :]
df_Houston_map_actual['tract'] = 's' + df_Houston_map_actual['TractFIPS']
df_Houston_map_actual.reset_index(drop=True, inplace=True)
df_Houston_map = pd.concat([df_Houston_map_actual, my_df_preds], axis=1)
df_Houston_map.rename(columns={'Population2010': 'POP2010'}, inplace=True)
# %%
...
for index, feature in enumerate(my_features):
    if feature in ['P_LOWWAGEr', 'P_LOWWAGEe', 'HHWRKJOBEQ', 'D_X_EXCLAO']:
        df_Houston_map['m_' + feature[2:]] = na_mean[index]
        df_Houston_map['s_' + feature[2:]] = na_sd[index]
    elif feature == 'WRKSPERJOB':
        df_Houston_map['m' + 'WKSPERJOB'] = na_mean[index]
        df_Houston_map['s' + 'WKSPERJOB'] = na_sd[index]
    else:
        df_Houston_map['m' + feature] = na_mean[index]
        df_Houston_map['s' + feature] = na_sd[index]
# %%
# save_csv(df_Houston_map, 'Houston_map_web')
# %%
...
save_csv(df_lasso_coef, "Houston_lasso_coef_stroke")
