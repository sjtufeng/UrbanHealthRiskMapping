...
# %%
filename_NYC_311_agg_tract = os.path.join(dirname, 'machine_learning/ml_NYC_311_agg_tract.csv')
df_NYC_311_agg_tract = pd.read_csv(filename_NYC_311_agg_tract, dtype={'tract2010': str})
# %%
ser_count311 = df_NYC_311_agg_tract.drop(columns=['tract2010']).sum()
...
c3 = ['Illegal Parking',
      'Blocked Driveway']
c4 = ['Request Large Bulky Item Collection',
      'Missed Collection (All Materials)']
c5 = ['UNSANITARY CONDITION',
      'Dirty Conditions',
      'Sanitation Condition',
      'Rodent']
c6 = ['Water System',
      'Sewer']
c7 = ['General Construction/Plumbing']
c8 = ['Street Light Condition',
      'Sidewalk Condition',
      'Street Condition']
c9 = ['Derelict Vehicles',
      'Derelict Vehicle']
c10 = ['Damaged Tree']
c11 = ['Building/Use']

df_NYC_311_agg_tract['agg_ALL'] = df_NYC_311_agg_tract.loc[:, all_311_types].sum(axis=1)
df_NYC_311_agg_tract['agg_noise'] = df_NYC_311_agg_tract.loc[:, c1].sum(axis=1)
df_NYC_311_agg_tract['agg_bldg_sys'] = df_NYC_311_agg_tract.loc[:, c2].sum(axis=1)
df_NYC_311_agg_tract['agg_parking'] = df_NYC_311_agg_tract.loc[:, c3].sum(axis=1)
df_NYC_311_agg_tract['agg_collection'] = df_NYC_311_agg_tract.loc[:, c4].sum(axis=1)
df_NYC_311_agg_tract['agg_sanitation'] = df_NYC_311_agg_tract.loc[:, c5].sum(axis=1)
df_NYC_311_agg_tract['agg_waterleak'] = df_NYC_311_agg_tract.loc[:, c6].sum(axis=1)
df_NYC_311_agg_tract['agg_construction'] = df_NYC_311_agg_tract.loc[:, c7].sum(axis=1)
df_NYC_311_agg_tract['agg_street'] = df_NYC_311_agg_tract.loc[:, c8].sum(axis=1)
df_NYC_311_agg_tract['agg_derelict_vehicle'] = df_NYC_311_agg_tract.loc[:, c9].sum(axis=1)
df_NYC_311_agg_tract['agg_dmg_tree'] = df_NYC_311_agg_tract.loc[:, c10].sum(axis=1)
df_NYC_311_agg_tract['agg_bldg_use'] = df_NYC_311_agg_tract.loc[:, c11].sum(axis=1)

df_NYC_311 = pd.concat([df_NYC_311_agg_tract[['tract2010']], df_NYC_311_agg_tract.loc[:, 'agg_ALL':]], axis=1)

# %%
filename_NYC_SLD_agg_tract = os.path.join(dirname, 'machine_learning/ml_NYC_SLD_agg_tract.csv')
df_NYC_SLD_agg_tract = pd.read_csv(filename_NYC_SLD_agg_tract, dtype={'tract_id': str})
# %%
cols_sld = ['tract_id', 'P_WRKAGE',
            ...
            'D4b025', 'D4b050', 'D4d']
df_NYC_SLD_agg_tract = df_NYC_SLD_agg_tract[cols_sld]
# %%
filename_NYC_SVI = os.path.join(dirname, 'machine_learning/ml_NYC_SVI.csv')
df_NYC_SVI = pd.read_csv(filename_NYC_SVI, dtype={'FIPS': str})
# %%
cols_svi = ['FIPS'] + [c for c in df_NYC_SVI.columns if c.startswith('EP_')]
df_NYC_SVI = df_NYC_SVI[cols_svi]
# %%
filename_NYC_CDC = os.path.join(dirname, 'machine_learning/ml_NYC_CDC.csv')
df_NYC_CDC = pd.read_csv(filename_NYC_CDC, dtype={'TractFIPS': str})
# %%
...
# %%
# merge data
d1 = pd.merge(df_NYC_CDC[['TractFIPS', 'Population2010'] + health_vars], df_NYC_311, left_on='TractFIPS',
              right_on='tract2010', how='left').drop(columns='tract2010')
d2 = pd.merge(d1, df_NYC_SLD_agg_tract, left_on='TractFIPS', right_on='tract_id', how='left').drop(
    columns='tract_id')
d3 = pd.merge(d2, df_NYC_SVI, left_on='TractFIPS', right_on='FIPS', how='left').drop(columns='FIPS')
df_NYC_all_data = d3
# %%
# adjust the 311 counts by population
df_NYC_all_data_adj = df_NYC_all_data.copy()
cols_311_types = df_NYC_311.columns.to_list()
cols_311_types.remove('tract2010')
df_NYC_all_data_adj[cols_311_types] = df_NYC_all_data_adj[cols_311_types].div(
    df_NYC_all_data_adj['Population2010'], axis=0)
# %%
# check null values
cols_nan = df_NYC_all_data_adj.columns[(df_NYC_all_data_adj.isnull().sum() != 0)].to_list()
df_NYC_all_data_adj[cols_nan].isnull().sum()
# %%
df_NYC_all_data_adj.dropna(subset=cols_nan, inplace=True)
# %%
df_NYC_tracts = df_NYC_all_data_adj[['TractFIPS']].copy()
df_NYC_pop = df_NYC_all_data_adj[['Population2010']].copy()
df_NYC_all_data_adj.drop(columns=['Population2010', 'TractFIPS'], inplace=True)

# %%
# shorten column names (<= 10 characters)
col_name_map = {'agg_ALL': 'SR_ALL', 'agg_noise': 'SR_NOISE', 'agg_bldg_sys': 'SR_BLDGSYS', 'agg_parking': 'SR_PRKNG',
                'agg_collection': 'SR_COLLECT', 'agg_sanitation': 'SR_SANIT', 'agg_waterleak': 'SR_WTRLEAK',
                'agg_construction': 'SR_CONSTR', 'agg_street': 'SR_STREET', 'agg_derelict_vehicle': 'SR_DERCVEH',
                'agg_dmg_tree': 'SR_DMGTREE', 'agg_bldg_use': 'SR_BLDGUSE',
...
df_NYC_all_data_adj.rename(columns=col_name_map, inplace=True)
...
# %%
train_set, test_set = train_test_split(df_NYC_all_data_adj, test_size=0.2, random_state=42)
...
df_NYC_map_actual = pd.concat([df_NYC_tracts, df_NYC_pop, df_NYC_all_data_adj], axis=1).loc[my_indices, :]
df_NYC_map_actual['tract'] = 's' + df_NYC_map_actual['TractFIPS']
df_NYC_map_actual.reset_index(drop=True, inplace=True)
df_NYC_map = pd.concat([df_NYC_map_actual, my_df_preds], axis=1)
df_NYC_map.rename(columns={'Population2010': 'POP2010'}, inplace=True)
# %%
...
for index, feature in enumerate(my_features):
    if feature in ['P_LOWWAGEr', 'P_LOWWAGEe', 'HHWRKJOBEQ', 'D_X_EXCLAO']:
        df_NYC_map['m_' + feature[2:]] = na_mean[index]
        df_NYC_map['s_' + feature[2:]] = na_sd[index]
    elif feature == 'WRKSPERJOB':
        df_NYC_map['m' + 'WKSPERJOB'] = na_mean[index]
        df_NYC_map['s' + 'WKSPERJOB'] = na_sd[index]
    else:
        df_NYC_map['m' + feature] = na_mean[index]
        df_NYC_map['s' + feature] = na_sd[index]
# %%
# save_csv(df_NYC_map, 'NYC_map_web')
# %%
...
save_csv(df_lasso_coef, "NYC_lasso_coef_stroke")
