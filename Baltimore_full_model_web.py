...
# %%
filename_Baltimore_311_agg_tract = os.path.join(dirname, 'machine_learning/ml_Baltimore_311_agg_tract.csv')
df_Baltimore_311_agg_tract = pd.read_csv(filename_Baltimore_311_agg_tract, dtype={'tract2010': str})
# %%
ser_count311 = df_Baltimore_311_agg_tract.drop(columns=['tract2010']).sum()
...
c1 = ['SW-Rat Rubout Proactive',
      ...
      'SW-Recycling',
      'SW-Rat Rubout']
c2 = ['TRS-Parking Complaint',
      'TRS-48 Hour Parking Complaint',
      'TRS-Abandoned Vehicle',
      'TRS-48 Hour Parking/Abandoned Vehicle',
      'TTR-Vehicle Removal']
c3 = ['BGE-StLight(s) Out',
      'TRM-Potholes',
      'TRM-Pickup Pothole',
      'TRT-Traffic Signal Repairs']
c4 = ['ECC-Vehicle Look Up']
c5 = ['ECC-Escalation']
c6 = ['WW-Water Leak (Exterior)',
      'WW-Sewer Water In Basement',
      'WW-Water No Water']
c7 = ['HCD-Vacant Building']
c8 = ['HCD-Maintenance Structure']
c9 = ['FIR-Fire Smoke Alarm Installation Request']
c10 = ['FOR-Tree Inspection']
c11 = ['HLTH-Animal In Danger/Injured/Abused/Neglected']
c12 = ['SW-Graffiti Removal',
       'SW-Graffiti Removal Proactive']

df_Baltimore_311_agg_tract['agg_ALL'] = df_Baltimore_311_agg_tract.loc[:, all_311_types].sum(axis=1)
df_Baltimore_311_agg_tract['agg_sanitation'] = df_Baltimore_311_agg_tract.loc[:, c1].sum(axis=1)
df_Baltimore_311_agg_tract['agg_parking'] = df_Baltimore_311_agg_tract.loc[:, c2].sum(axis=1)
df_Baltimore_311_agg_tract['agg_street'] = df_Baltimore_311_agg_tract.loc[:, c3].sum(axis=1)
df_Baltimore_311_agg_tract['agg_vehicle_lookup'] = df_Baltimore_311_agg_tract.loc[:, c4].sum(axis=1)
df_Baltimore_311_agg_tract['agg_escalation'] = df_Baltimore_311_agg_tract.loc[:, c5].sum(axis=1)
df_Baltimore_311_agg_tract['agg_water'] = df_Baltimore_311_agg_tract.loc[:, c6].sum(axis=1)
df_Baltimore_311_agg_tract['agg_vacant_bldg'] = df_Baltimore_311_agg_tract.loc[:, c7].sum(axis=1)
df_Baltimore_311_agg_tract['agg_maintenance'] = df_Baltimore_311_agg_tract.loc[:, c8].sum(axis=1)
df_Baltimore_311_agg_tract['agg_alarm_install'] = df_Baltimore_311_agg_tract.loc[:, c9].sum(axis=1)
df_Baltimore_311_agg_tract['agg_tree'] = df_Baltimore_311_agg_tract.loc[:, c10].sum(axis=1)
df_Baltimore_311_agg_tract['agg_animal'] = df_Baltimore_311_agg_tract.loc[:, c11].sum(axis=1)
df_Baltimore_311_agg_tract['agg_graffiti'] = df_Baltimore_311_agg_tract.loc[:, c12].sum(axis=1)

df_Baltimore_311 = pd.concat([df_Baltimore_311_agg_tract[['tract2010']], df_Baltimore_311_agg_tract.loc[:, 'agg_ALL':]],
                             axis=1)
# %%
filename_Baltimore_SLD_agg_tract = os.path.join(dirname, 'machine_learning/ml_Baltimore_SLD_agg_tract.csv')
df_Baltimore_SLD_agg_tract = pd.read_csv(filename_Baltimore_SLD_agg_tract, dtype={'tract_id': str})
# %%
cols_sld = ['tract_id', 'P_WRKAGE',
            ...
            'D4b025', 'D4b050', 'D4d']
df_Baltimore_SLD_agg_tract = df_Baltimore_SLD_agg_tract[cols_sld]
# %%
filename_Baltimore_SVI = os.path.join(dirname, 'machine_learning/ml_Baltimore_SVI.csv')
df_Baltimore_SVI = pd.read_csv(filename_Baltimore_SVI, dtype={'FIPS': str})
# %%
cols_svi = ['FIPS'] + [c for c in df_Baltimore_SVI.columns if c.startswith('EP_')]
df_Baltimore_SVI = df_Baltimore_SVI[cols_svi]
# %%
filename_Baltimore_CDC = os.path.join(dirname, 'machine_learning/ml_Baltimore_CDC.csv')
df_Baltimore_CDC = pd.read_csv(filename_Baltimore_CDC, dtype={'TractFIPS': str})
# %%
...
# %%
# merge data
d1 = pd.merge(df_Baltimore_CDC[['TractFIPS', 'Population2010'] + health_vars], df_Baltimore_311, left_on='TractFIPS',
              right_on='tract2010', how='left').drop(columns='tract2010')
d2 = pd.merge(d1, df_Baltimore_SLD_agg_tract, left_on='TractFIPS', right_on='tract_id', how='left').drop(
    columns='tract_id')
d3 = pd.merge(d2, df_Baltimore_SVI, left_on='TractFIPS', right_on='FIPS', how='left').drop(columns='FIPS')
df_Baltimore_all_data = d3
# %%
# adjust the 311 counts by population
df_Baltimore_all_data_adj = df_Baltimore_all_data.copy()
cols_311_types = df_Baltimore_311.columns.to_list()
cols_311_types.remove('tract2010')
df_Baltimore_all_data_adj[cols_311_types] = df_Baltimore_all_data_adj[cols_311_types].div(
    df_Baltimore_all_data_adj['Population2010'], axis=0)
# %%
# check null values
cols_nan = df_Baltimore_all_data_adj.columns[(df_Baltimore_all_data_adj.isnull().sum() != 0)].to_list()
df_Baltimore_all_data_adj[cols_nan].isnull().sum()
# %%
df_Baltimore_all_data_adj.dropna(subset=cols_nan, inplace=True)
# %%
df_Baltimore_tracts = df_Baltimore_all_data_adj[['TractFIPS']].copy()
df_Baltimore_pop = df_Baltimore_all_data_adj[['Population2010']].copy()
df_Baltimore_all_data_adj.drop(columns=['Population2010', 'TractFIPS'], inplace=True)

# %%
# shorten column names (<= 10 characters)
col_name_map = {'agg_ALL': 'SR_ALL', 'agg_sanitation': 'SR_SANIT', 'agg_parking': 'SR_PRKNG', 'agg_street': 'SR_STREET',
                'agg_vehicle_lookup': 'SR_VEH', 'agg_escalation': 'SR_ESCALAT', 'agg_water': 'SR_WATER',
                'agg_vacant_bldg': 'SR_VACBLDG', 'agg_maintenance': 'SR_MAINT', 'agg_alarm_install': 'SR_ALARM',
                'agg_tree': 'SR_TREE', 'agg_animal': 'SR_ANIMAL', 'agg_graffiti': 'SR_GRAFFIT',
...
df_Baltimore_all_data_adj.rename(columns=col_name_map, inplace=True)
...
# %%
train_set, test_set = train_test_split(df_Baltimore_all_data_adj, test_size=0.2, random_state=42)
...
df_Baltimore_map_actual = pd.concat([df_Baltimore_tracts, df_Baltimore_pop, df_Baltimore_all_data_adj], axis=1).loc[
                          my_indices, :]
df_Baltimore_map_actual['tract'] = 's' + df_Baltimore_map_actual['TractFIPS']
df_Baltimore_map_actual.reset_index(drop=True, inplace=True)
df_Baltimore_map = pd.concat([df_Baltimore_map_actual, my_df_preds], axis=1)
df_Baltimore_map.rename(columns={'Population2010': 'POP2010'}, inplace=True)
# %%
...
for index, feature in enumerate(my_features):
    if feature in ['P_LOWWAGEr', 'P_LOWWAGEe', 'HHWRKJOBEQ', 'D_X_EXCLAO']:
        df_Baltimore_map['m_' + feature[2:]] = na_mean[index]
        df_Baltimore_map['s_' + feature[2:]] = na_sd[index]
    elif feature == 'WRKSPERJOB':
        df_Baltimore_map['m' + 'WKSPERJOB'] = na_mean[index]
        df_Baltimore_map['s' + 'WKSPERJOB'] = na_sd[index]
    else:
        df_Baltimore_map['m' + feature] = na_mean[index]
        df_Baltimore_map['s' + feature] = na_sd[index]
# %%
# save_csv(df_Baltimore_map, 'Baltimore_map_web')
# %%
...
save_csv(df_lasso_coef, "Baltimore_lasso_coef_stroke")
