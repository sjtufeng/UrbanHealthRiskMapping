...
# %%
filename_SanAntonio_311_agg_tract = os.path.join(dirname, 'machine_learning/ml_SanAntonio_311_agg_tract.csv')
df_SanAntonio_311_agg_tract = pd.read_csv(filename_SanAntonio_311_agg_tract, dtype={'tract2010': str})
# %%
ser_count311 = df_SanAntonio_311_agg_tract.drop(columns=['tract2010']).sum()
...
# %%
# feature engineering
c1 = ['Overgrown Yard/Trash',
      'Brush Violations',
      'Brush - Out of Cycle Collection - Fee Required',
      'Solid Waste Fees and Charges',
      '1st Call Bagged Leaves',
      'No Pickup',
      'Illegal Dumping (Non- residential areas)',
...
c3 = ['Cart Exchange Request',
      'Damaged Cart',
      'Cart Delivery',
      'Lost/Stolen Cart',
      'Cart PickUp']
c4 = ['Low Hanging Limbs on Streets/Sidewalk',
      'Alley-Way Maintenance',
      'Pot Hole Repair',
      'Right Of Way/Sidewalk Obstruction',
      'Base/Pavement Repair']
c5 = ['Front Or Side Yard Parking',
      'Junk Vehicle On Private Property']
c6 = ['Traffic Sign Faded',
      'Traffic Signal Ops and Maintenance - Emergency',
      'Traffic Signal Ops and Maintenance',
      'Signal Timing Modification',
      'Traffic Sign Missing/Down']
c7 = ['Traffic Sign Graffiti',
      'Graffiti: Private Property (Corridors)',
      'Graffiti-Private Property']
c8 = ['Scooter Complaint',
      'Scooter Complaint - High Priority']
c9 = ['Minimum Housing-Owner Occupied',
      'Minimum Housing-Tenant Occupied']
c10 = ['Vacant House Overgrown Yard',
       'Vacant Lot-Private (12")',
       'Vacant Lot/Overgrown/Trash']
c11 = ['Bandit Signs']
c12 = ['Permits Building']
c13 = ['Dang. Premises/CL/SEC']
c14 = ['Initiating Unknown Project Investigation']
c15 = ['Complaint']

df_SanAntonio_311_agg_tract['agg_ALL'] = df_SanAntonio_311_agg_tract.loc[:, all_311_types].sum(axis=1)
df_SanAntonio_311_agg_tract['agg_trash'] = df_SanAntonio_311_agg_tract.loc[:, c1].sum(axis=1)
df_SanAntonio_311_agg_tract['agg_animal'] = df_SanAntonio_311_agg_tract.loc[:, c2].sum(axis=1)
df_SanAntonio_311_agg_tract['agg_cart'] = df_SanAntonio_311_agg_tract.loc[:, c3].sum(axis=1)
df_SanAntonio_311_agg_tract['agg_street'] = df_SanAntonio_311_agg_tract.loc[:, c4].sum(axis=1)
df_SanAntonio_311_agg_tract['agg_parking'] = df_SanAntonio_311_agg_tract.loc[:, c5].sum(axis=1)
df_SanAntonio_311_agg_tract['agg_traffic_sign'] = df_SanAntonio_311_agg_tract.loc[:, c6].sum(axis=1)
df_SanAntonio_311_agg_tract['agg_graffiti'] = df_SanAntonio_311_agg_tract.loc[:, c7].sum(axis=1)
df_SanAntonio_311_agg_tract['agg_scooter'] = df_SanAntonio_311_agg_tract.loc[:, c8].sum(axis=1)
df_SanAntonio_311_agg_tract['agg_min_occupied'] = df_SanAntonio_311_agg_tract.loc[:, c9].sum(axis=1)
df_SanAntonio_311_agg_tract['agg_vacant_lot'] = df_SanAntonio_311_agg_tract.loc[:, c10].sum(axis=1)
df_SanAntonio_311_agg_tract['agg_bandit_sign'] = df_SanAntonio_311_agg_tract.loc[:, c11].sum(axis=1)
df_SanAntonio_311_agg_tract['agg_permit_building'] = df_SanAntonio_311_agg_tract.loc[:, c12].sum(axis=1)
df_SanAntonio_311_agg_tract['agg_premise_cl_sec'] = df_SanAntonio_311_agg_tract.loc[:, c13].sum(axis=1)
df_SanAntonio_311_agg_tract['agg_unknown'] = df_SanAntonio_311_agg_tract.loc[:, c14].sum(axis=1)
df_SanAntonio_311_agg_tract['agg_complaint'] = df_SanAntonio_311_agg_tract.loc[:, c15].sum(axis=1)

df_SanAntonio_311 = pd.concat(
    [df_SanAntonio_311_agg_tract[['tract2010']], df_SanAntonio_311_agg_tract.loc[:, 'agg_ALL':]], axis=1)

# %%
filename_SanAntonio_SLD_agg_tract = os.path.join(dirname, 'machine_learning/ml_SanAntonio_SLD_agg_tract.csv')
df_SanAntonio_SLD_agg_tract = pd.read_csv(filename_SanAntonio_SLD_agg_tract, dtype={'tract_id': str})
# %%
cols_sld = ['tract_id', 'P_WRKAGE',
            ...
            'D4b025', 'D4b050', 'D4d']
df_SanAntonio_SLD_agg_tract = df_SanAntonio_SLD_agg_tract[cols_sld]
# %%
filename_SanAntonio_SVI = os.path.join(dirname, 'machine_learning/ml_SanAntonio_SVI.csv')
df_SanAntonio_SVI = pd.read_csv(filename_SanAntonio_SVI, dtype={'FIPS': str})
# %%
cols_svi = ['FIPS'] + [c for c in df_SanAntonio_SVI.columns if c.startswith('EP_')]
df_SanAntonio_SVI = df_SanAntonio_SVI[cols_svi]
# %%
filename_SanAntonio_CDC = os.path.join(dirname, 'machine_learning/ml_SanAntonio_CDC.csv')
df_SanAntonio_CDC = pd.read_csv(filename_SanAntonio_CDC, dtype={'TractFIPS': str})
# %%
...
# %%
# merge data
d1 = pd.merge(df_SanAntonio_CDC[['TractFIPS', 'Population2010'] + health_vars], df_SanAntonio_311, left_on='TractFIPS',
              right_on='tract2010', how='left').drop(columns='tract2010')
d2 = pd.merge(d1, df_SanAntonio_SLD_agg_tract, left_on='TractFIPS', right_on='tract_id', how='left').drop(
    columns='tract_id')
d3 = pd.merge(d2, df_SanAntonio_SVI, left_on='TractFIPS', right_on='FIPS', how='left').drop(columns='FIPS')
df_SanAntonio_all_data = d3
# %%
# adjust the 311 counts by population
df_SanAntonio_all_data_adj = df_SanAntonio_all_data.copy()
cols_311_types = df_SanAntonio_311.columns.to_list()
cols_311_types.remove('tract2010')
df_SanAntonio_all_data_adj[cols_311_types] = df_SanAntonio_all_data_adj[cols_311_types].div(
    df_SanAntonio_all_data_adj['Population2010'], axis=0)
# %%
# check null values
cols_nan = df_SanAntonio_all_data_adj.columns[(df_SanAntonio_all_data_adj.isnull().sum() != 0)].to_list()
df_SanAntonio_all_data_adj[cols_nan].isnull().sum()
# %%
df_SanAntonio_all_data_adj.drop(columns=['D4d'], inplace=True)
# %%
cols_nan.remove('D4d')
df_SanAntonio_all_data_adj.dropna(subset=cols_nan, inplace=True)
# %%
df_SanAntonio_tracts = df_SanAntonio_all_data_adj[['TractFIPS']].copy()
df_SanAntonio_pop = df_SanAntonio_all_data_adj[['Population2010']].copy()
df_SanAntonio_all_data_adj.drop(columns=['Population2010', 'TractFIPS'], inplace=True)

# %%
# shorten column names (<= 10 characters)
col_name_map = {'agg_ALL': 'SR_ALL', 'agg_trash': 'SR_TRASH', 'agg_animal': 'SR_ANIMAL', 'agg_cart': 'SR_CART',
                'agg_street': 'SR_STREET', 'agg_parking': 'SR_PRKNG', 'agg_traffic_sign': 'SR_TRFSIGN',
                'agg_graffiti': 'SR_GRAFFIT', 'agg_scooter': 'SR_SCOOTER', 'agg_min_occupied': 'SR_MINOCCP',
                'agg_vacant_lot': 'SR_VACLOT', 'agg_bandit_sign': 'SR_BANDIT', 'agg_permit_building': 'SR_PMTBLDG',
                'agg_premise_cl_sec': 'SR_PREMISE', 'agg_unknown': 'SR_UNKNOWN', 'agg_complaint': 'SR_COMPLNT',
...
df_SanAntonio_all_data_adj.rename(columns=col_name_map, inplace=True)
...
# %%
train_set, test_set = train_test_split(df_SanAntonio_all_data_adj, test_size=0.2, random_state=42)
...
df_SanAntonio_map_actual = pd.concat([df_SanAntonio_tracts, df_SanAntonio_pop, df_SanAntonio_all_data_adj], axis=1).loc[
                           my_indices, :]
df_SanAntonio_map_actual['tract'] = 's' + df_SanAntonio_map_actual['TractFIPS']
df_SanAntonio_map_actual.reset_index(drop=True, inplace=True)
df_SanAntonio_map = pd.concat([df_SanAntonio_map_actual, my_df_preds], axis=1)
df_SanAntonio_map.rename(columns={'Population2010': 'POP2010'}, inplace=True)
# %%
...
for index, feature in enumerate(my_features):
    if feature in ['P_LOWWAGEr', 'P_LOWWAGEe', 'HHWRKJOBEQ', 'D_X_EXCLAO']:
        df_SanAntonio_map['m_' + feature[2:]] = na_mean[index]
        df_SanAntonio_map['s_' + feature[2:]] = na_sd[index]
    elif feature == 'WRKSPERJOB':
        df_SanAntonio_map['m' + 'WKSPERJOB'] = na_mean[index]
        df_SanAntonio_map['s' + 'WKSPERJOB'] = na_sd[index]
    else:
        df_SanAntonio_map['m' + feature] = na_mean[index]
        df_SanAntonio_map['s' + feature] = na_sd[index]
# %%
# save_csv(df_SanAntonio_map, 'SanAntonio_map_web')
# %%
...
save_csv(df_lasso_coef, "SanAntonio_lasso_coef_stroke")
