...
# %%
filename_Dallas_311_agg_tract = os.path.join(dirname, 'machine_learning/ml_Dallas_311_agg_tract.csv')
df_Dallas_311_agg_tract = pd.read_csv(filename_Dallas_311_agg_tract, dtype={'tract2010': str})
# %%
ser_count311 = df_Dallas_311_agg_tract.drop(columns=['tract2010']).sum()
...
# feature engineering
c1 = ['Animal Loose - DAS',
      'Dead Animal Pick Up - DAS',
      'Animal Lack of Care - DAS',
      'Animal Aggressive Behavior - DAS',
      'Animal Confined - DAS',
      'Animal Nuisance - DAS']
c2 = ['Parking - Report a Violation - TRN',
      '24 Hour Parking Violation - DPD',
      'Parking Unapproved Surface - CCS']
c3 = ['Sanitation Roll Cart Maintenance/Delivery - SAN',
      'Sanitation Missed Garbage - SAN',
      'Sanitation Missed Service - SAN',
      'Sanitation Compliment/Complaint - SAN',
      'Sanitation Missed Recycle - SAN',
      'Sanitation Billing - SAN',
      'Sanitation Same Day Missed Collection - SAN',
      'Litter - CCS',
      'Bulky Trash Violations - CCS',
      'Illegal Dumping - CCS',
      'Storm Brush/Debris - SAN',
      'Brush Busters - SAN',
      'Cost Plus - SAN']
c4 = ['Signs�- CCS',
      'Traffic Signal - Flashing - TRN',
      'Traffic Sign - Maintenance (Other) - TRN',
      'Traffic Signal - Other Dispatch - TRN']
c5 = ['Obstruction Alley/Sidewalk/Street - CCS',
      'Street Repair - Routine - PBW',
      'Alley/Sidewalk/Street Repair - Hazardous - PBW',
      'Pot Hole - Routine - PBW',
      'Pot Hole - Hazardous - PBW',
      'Alley Repair - Routine - PBW']
c6 = ['Water/Wastewater Line Locate - 311',
      'Water Questions - 311']
c7 = ['Extra Police Patrol - DPD']
c8 = ['High Weeds - CCS', 'High Weeds']
c9 = ['Substandard Structure - CCS']
c10 = ['Single Family Rental Needs Registration - CCS']
c11 = ['Graffiti Private Property - CCS']
c12 = ['Illegal Land Use - CCS', 'Illegal Outside Storage - CCS']
c13 = ['Code General - CCS']
c14 = ['Homeless Encampment - OHS']
c15 = ['Tree Down/Low Limbs - PBW']
c16 = ['Complaint/Compliment - 311']

df_Dallas_311_agg_tract['agg_ALL'] = df_Dallas_311_agg_tract.loc[:, all_311_types].sum(axis=1)
df_Dallas_311_agg_tract['agg_animal'] = df_Dallas_311_agg_tract.loc[:, c1].sum(axis=1)
df_Dallas_311_agg_tract['agg_parking'] = df_Dallas_311_agg_tract.loc[:, c2].sum(axis=1)
df_Dallas_311_agg_tract['agg_sanitation'] = df_Dallas_311_agg_tract.loc[:, c3].sum(axis=1)
df_Dallas_311_agg_tract['agg_sign_signal'] = df_Dallas_311_agg_tract.loc[:, c4].sum(axis=1)
df_Dallas_311_agg_tract['agg_street'] = df_Dallas_311_agg_tract.loc[:, c5].sum(axis=1)
df_Dallas_311_agg_tract['agg_water'] = df_Dallas_311_agg_tract.loc[:, c6].sum(axis=1)
df_Dallas_311_agg_tract['agg_police'] = df_Dallas_311_agg_tract.loc[:, c7].sum(axis=1)
df_Dallas_311_agg_tract['agg_high_weeds'] = df_Dallas_311_agg_tract.loc[:, c8].sum(axis=1)
df_Dallas_311_agg_tract['agg_substandard_structure'] = df_Dallas_311_agg_tract.loc[:, c9].sum(axis=1)
df_Dallas_311_agg_tract['agg_rental_registration'] = df_Dallas_311_agg_tract.loc[:, c10].sum(axis=1)
df_Dallas_311_agg_tract['agg_graffiti'] = df_Dallas_311_agg_tract.loc[:, c11].sum(axis=1)
df_Dallas_311_agg_tract['agg_illegal_landuse'] = df_Dallas_311_agg_tract.loc[:, c12].sum(axis=1)
df_Dallas_311_agg_tract['agg_code'] = df_Dallas_311_agg_tract.loc[:, c13].sum(axis=1)
df_Dallas_311_agg_tract['agg_homeless'] = df_Dallas_311_agg_tract.loc[:, c14].sum(axis=1)
df_Dallas_311_agg_tract['agg_tree'] = df_Dallas_311_agg_tract.loc[:, c15].sum(axis=1)
df_Dallas_311_agg_tract['agg_311complaint'] = df_Dallas_311_agg_tract.loc[:, c16].sum(axis=1)

df_Dallas_311 = pd.concat([df_Dallas_311_agg_tract[['tract2010']], df_Dallas_311_agg_tract.loc[:, 'agg_ALL':]],
                          axis=1)

# %%
filename_Dallas_SLD_agg_tract = os.path.join(dirname, 'machine_learning/ml_Dallas_SLD_agg_tract.csv')
df_Dallas_SLD_agg_tract = pd.read_csv(filename_Dallas_SLD_agg_tract, dtype={'tract_id': str})
# %%
cols_sld = ['tract_id', 'P_WRKAGE',
            ...
            'D4b025', 'D4b050', 'D4d']
df_Dallas_SLD_agg_tract = df_Dallas_SLD_agg_tract[cols_sld]
# %%
filename_Dallas_SVI = os.path.join(dirname, 'machine_learning/ml_Dallas_SVI.csv')
df_Dallas_SVI = pd.read_csv(filename_Dallas_SVI, dtype={'FIPS': str})
# %%
cols_svi = ['FIPS'] + [c for c in df_Dallas_SVI.columns if c.startswith('EP_')]
df_Dallas_SVI = df_Dallas_SVI[cols_svi]
# %%
filename_Dallas_CDC = os.path.join(dirname, 'machine_learning/ml_Dallas_CDC.csv')
df_Dallas_CDC = pd.read_csv(filename_Dallas_CDC, dtype={'TractFIPS': str})
# %%
...
# %%
# merge data
d1 = pd.merge(df_Dallas_CDC[['TractFIPS', 'Population2010'] + health_vars], df_Dallas_311, left_on='TractFIPS',
              right_on='tract2010', how='left').drop(columns='tract2010')
d2 = pd.merge(d1, df_Dallas_SLD_agg_tract, left_on='TractFIPS', right_on='tract_id', how='left').drop(
    columns='tract_id')
d3 = pd.merge(d2, df_Dallas_SVI, left_on='TractFIPS', right_on='FIPS', how='left').drop(columns='FIPS')
df_Dallas_all_data = d3
# %%
# adjust the 311 counts by population
df_Dallas_all_data_adj = df_Dallas_all_data.copy()
cols_311_types = df_Dallas_311.columns.to_list()
cols_311_types.remove('tract2010')
df_Dallas_all_data_adj[cols_311_types] = df_Dallas_all_data_adj[cols_311_types].div(
    df_Dallas_all_data_adj['Population2010'], axis=0)
# %%
# check null values
cols_nan = df_Dallas_all_data_adj.columns[(df_Dallas_all_data_adj.isnull().sum() != 0)].to_list()
df_Dallas_all_data_adj[cols_nan].isnull().sum()
# %%
df_Dallas_all_data_adj.dropna(subset=cols_nan, inplace=True)
# %%
df_Dallas_tracts = df_Dallas_all_data_adj[['TractFIPS']].copy()
df_Dallas_pop = df_Dallas_all_data_adj[['Population2010']].copy()
df_Dallas_all_data_adj.drop(columns=['Population2010', 'TractFIPS'], inplace=True)

# %%
# shorten column names (<= 10 characters)
col_name_map = {'agg_ALL': 'SR_ALL', 'agg_animal': 'SR_ANIMAL', 'agg_parking': 'SR_PRKNG', 'agg_sanitation': 'SR_SANIT',
                'agg_sign_signal': 'SR_SIGN', 'agg_street': 'SR_STREET', 'agg_water': 'SR_WATER',
                'agg_police': 'SR_POLICE', 'agg_high_weeds': 'SR_HGHWEED', 'agg_substandard_structure': 'SR_SUBSTR',
                'agg_rental_registration': 'SR_RENTREG', 'agg_graffiti': 'SR_GRAFFIT',
                'agg_illegal_landuse': 'SR_ILLEGLU', 'agg_code': 'SR_CODE', 'agg_homeless': 'SR_HMLESS',
                'agg_tree': 'SR_TREE', 'agg_311complaint': 'SR_311COMP',
...
df_Dallas_all_data_adj.rename(columns=col_name_map, inplace=True)
...
# %%
train_set, test_set = train_test_split(df_Dallas_all_data_adj, test_size=0.2, random_state=42)
...
df_Dallas_map_actual = pd.concat([df_Dallas_tracts, df_Dallas_pop, df_Dallas_all_data_adj], axis=1).loc[my_indices, :]
df_Dallas_map_actual['tract'] = 's' + df_Dallas_map_actual['TractFIPS']
df_Dallas_map_actual.reset_index(drop=True, inplace=True)
df_Dallas_map = pd.concat([df_Dallas_map_actual, my_df_preds], axis=1)
df_Dallas_map.rename(columns={'Population2010': 'POP2010'}, inplace=True)
# %%
...
for index, feature in enumerate(my_features):
    if feature in ['P_LOWWAGEr', 'P_LOWWAGEe', 'HHWRKJOBEQ', 'D_X_EXCLAO']:
        df_Dallas_map['m_' + feature[2:]] = na_mean[index]
        df_Dallas_map['s_' + feature[2:]] = na_sd[index]
    elif feature == 'WRKSPERJOB':
        df_Dallas_map['m' + 'WKSPERJOB'] = na_mean[index]
        df_Dallas_map['s' + 'WKSPERJOB'] = na_sd[index]
    else:
        df_Dallas_map['m' + feature] = na_mean[index]
        df_Dallas_map['s' + feature] = na_sd[index]
# %%
# save_csv(df_Dallas_map, 'Dallas_map_web')
# %%
...
save_csv(df_lasso_coef, "Dallas_lasso_coef_stroke")
