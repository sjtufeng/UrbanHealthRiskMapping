...
# %%
filename_Austin_311_agg_tract = os.path.join(dirname, 'machine_learning/ml_Austin_311_agg_tract.csv')
df_Austin_311_agg_tract = pd.read_csv(filename_Austin_311_agg_tract, dtype={'tract2010': str})
# %%
ser_count311 = df_Austin_311_agg_tract.drop(columns=['tract2010']).sum()
...
# %%
# feature engineering
c1 = ['Austin Code - Request Code Officer',
      'Code Compliance',
      'Austin Code - Short Term Rental Complaint SR']
c2 = ['Loud Commercial Music', 'Loud Music']
c3 = ['Graffiti Abatement', 'Public Health - Graffiti Abatement']
c4 = ['Animal Control - Assistance Request',
      'Injured / Sick Animal',
      'Animal - Proper Care',
...
c5 = ['Traffic Signal - Maintenance',
      'Traffic Signal - Dig Tess Request',
...
c6 = ['Debris in Street',
      ...
      'Sidewalk Repair']
c7 = ['ARR Missed Yard Trimmings/Compost',
      'ARR Missed Recycling',
      'ARR Missed Yard Trimmings /Organics',
      'ARR Brush and Bulk']
c8 = ['Water Waste Report']
c9 = ['Park Maintenance - Grounds']

df_Austin_311_agg_tract['agg_ALL'] = df_Austin_311_agg_tract.loc[:, all_311_types].sum(axis=1)
df_Austin_311_agg_tract['agg_code'] = df_Austin_311_agg_tract.loc[:, c1].sum(axis=1)
df_Austin_311_agg_tract['agg_loud_music'] = df_Austin_311_agg_tract.loc[:, c2].sum(axis=1)
df_Austin_311_agg_tract['agg_graffiti'] = df_Austin_311_agg_tract.loc[:, c3].sum(axis=1)
df_Austin_311_agg_tract['agg_animal'] = df_Austin_311_agg_tract.loc[:, c4].sum(axis=1)
df_Austin_311_agg_tract['agg_sign_signal'] = df_Austin_311_agg_tract.loc[:, c5].sum(axis=1)
df_Austin_311_agg_tract['agg_street'] = df_Austin_311_agg_tract.loc[:, c6].sum(axis=1)
df_Austin_311_agg_tract['agg_recycling'] = df_Austin_311_agg_tract.loc[:, c7].sum(axis=1)
df_Austin_311_agg_tract['agg_water_waste'] = df_Austin_311_agg_tract.loc[:, c8].sum(axis=1)
df_Austin_311_agg_tract['agg_park_maintenance'] = df_Austin_311_agg_tract.loc[:, c9].sum(axis=1)

df_Austin_311 = pd.concat([df_Austin_311_agg_tract[['tract2010']], df_Austin_311_agg_tract.loc[:, 'agg_ALL':]], axis=1)
# %%
filename_Austin_SLD_agg_tract = os.path.join(dirname, 'machine_learning/ml_Austin_SLD_agg_tract.csv')
df_Austin_SLD_agg_tract = pd.read_csv(filename_Austin_SLD_agg_tract, dtype={'tract_id': str})
# %%
cols_sld = ['tract_id', 'P_WRKAGE',
            ...
            'D4b025', 'D4b050', 'D4d']
df_Austin_SLD_agg_tract = df_Austin_SLD_agg_tract[cols_sld]
# %%
filename_Austin_SVI = os.path.join(dirname, 'machine_learning/ml_Austin_SVI.csv')
df_Austin_SVI = pd.read_csv(filename_Austin_SVI, dtype={'FIPS': str})
# %%
cols_svi = ['FIPS'] + [c for c in df_Austin_SVI.columns if c.startswith('EP_')]
df_Austin_SVI = df_Austin_SVI[cols_svi]
# %%
filename_Austin_CDC = os.path.join(dirname, 'machine_learning/ml_Austin_CDC.csv')
df_Austin_CDC = pd.read_csv(filename_Austin_CDC, dtype={'TractFIPS': str})
# %%
...
# %%
# merge data
d1 = pd.merge(df_Austin_CDC[['TractFIPS', 'Population2010'] + health_vars], df_Austin_311, left_on='TractFIPS',
              right_on='tract2010', how='left').drop(columns='tract2010')
d2 = pd.merge(d1, df_Austin_SLD_agg_tract, left_on='TractFIPS', right_on='tract_id', how='left').drop(
    columns='tract_id')
d3 = pd.merge(d2, df_Austin_SVI, left_on='TractFIPS', right_on='FIPS', how='left').drop(columns='FIPS')
df_Austin_all_data = d3
# %%
# adjust the 311 counts by population
df_Austin_all_data_adj = df_Austin_all_data.copy()
cols_311_types = df_Austin_311.columns.to_list()
cols_311_types.remove('tract2010')
df_Austin_all_data_adj[cols_311_types] = df_Austin_all_data_adj[cols_311_types].div(
    df_Austin_all_data_adj['Population2010'], axis=0)
# %%
# check null values
cols_nan = df_Austin_all_data_adj.columns[(df_Austin_all_data_adj.isnull().sum() != 0)].to_list()
df_Austin_all_data_adj[cols_nan].isnull().sum()
# %%
df_Austin_all_data_adj.dropna(subset=cols_nan, inplace=True)
# %%
df_Austin_tracts = df_Austin_all_data_adj[['TractFIPS']].copy()
df_Austin_pop = df_Austin_all_data_adj[['Population2010']].copy()
df_Austin_all_data_adj.drop(columns=['Population2010', 'TractFIPS'], inplace=True)

# %%
# shorten column names (<= 10 characters)
col_name_map = {'agg_ALL': 'SR_ALL', 'agg_code': 'SR_CODE', 'agg_loud_music': 'SR_NOISE',
                'agg_graffiti': 'SR_GRAFFI', 'agg_animal': 'SR_ANIMAL', 'agg_sign_signal': 'SR_SIGN',
                'agg_street': 'SR_STREET', 'agg_recycling': 'SR_RECYCL', 'agg_water_waste': 'SR_WTRWTE',
                'agg_park_maintenance': 'SR_PRKMNT',
...
df_Austin_all_data_adj.rename(columns=col_name_map, inplace=True)
...
# %%
train_set, test_set = train_test_split(df_Austin_all_data_adj, test_size=0.2, random_state=42)
...
df_Austin_map_actual = pd.concat([df_Austin_tracts, df_Austin_pop, df_Austin_all_data_adj], axis=1).loc[my_indices, :]
df_Austin_map_actual['tract'] = 's' + df_Austin_map_actual['TractFIPS']
df_Austin_map_actual.reset_index(drop=True, inplace=True)
df_Austin_map = pd.concat([df_Austin_map_actual, my_df_preds], axis=1)
df_Austin_map.rename(columns={'Population2010': 'POP2010'}, inplace=True)
...
for index, feature in enumerate(my_features):
    if feature in ['P_LOWWAGEr', 'P_LOWWAGEe', 'HHWRKJOBEQ', 'D_X_EXCLAO']:
        df_Austin_map['m_' + feature[2:]] = na_mean[index]
        df_Austin_map['s_' + feature[2:]] = na_sd[index]
    elif feature == 'WRKSPERJOB':
        df_Austin_map['m' + 'WKSPERJOB'] = na_mean[index]
        df_Austin_map['s' + 'WKSPERJOB'] = na_sd[index]
    else:
        df_Austin_map['m' + feature] = na_mean[index]
        df_Austin_map['s' + feature] = na_sd[index]

# %%
# save_csv(df_Austin_map, 'Austin_map_web')
...
save_csv(df_lasso_coef, "Austin_lasso_coef_stroke")
