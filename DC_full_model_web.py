...
# %%
filename_DC_311_agg_tract = os.path.join(dirname, 'machine_learning/ml_DC_311_agg_tract.csv')
df_DC_311_agg_tract = pd.read_csv(filename_DC_311_agg_tract, dtype={'tract2010': str})
# %%
ser_count311 = df_DC_311_agg_tract.drop(columns=['tract2010']).sum()
...
c2 = ['Parking Enforcement',
      'Parking Meter Repair',
      'Emergency No-Parking Verification',
      'Residential Parking Permit Violation']
c3 = ['Roadway Signs']
c4 = ['Streetlight Repair Investigation',
      'Pothole',
      'Street Cleaning',
      'Alley Cleaning',
      'Sidewalk Repair']
c5 = ['Sanitation Enforcement',
      'Rodent Inspection and Treatment']
c6 = ['Abandoned Vehicle - On Public Property']
c7 = ['Tree Inspection']
c8 = ['Graffiti Removal']

df_DC_311_agg_tract['agg_ALL'] = df_DC_311_agg_tract.loc[:, all_311_types].sum(axis=1)
df_DC_311_agg_tract['agg_collection'] = df_DC_311_agg_tract.loc[:, c1].sum(axis=1)
df_DC_311_agg_tract['agg_parking'] = df_DC_311_agg_tract.loc[:, c2].sum(axis=1)
df_DC_311_agg_tract['agg_signs'] = df_DC_311_agg_tract.loc[:, c3].sum(axis=1)
df_DC_311_agg_tract['agg_street'] = df_DC_311_agg_tract.loc[:, c4].sum(axis=1)
df_DC_311_agg_tract['agg_sanitation'] = df_DC_311_agg_tract.loc[:, c5].sum(axis=1)
df_DC_311_agg_tract['agg_abandoned_vehicle'] = df_DC_311_agg_tract.loc[:, c6].sum(axis=1)
df_DC_311_agg_tract['agg_tree'] = df_DC_311_agg_tract.loc[:, c7].sum(axis=1)
df_DC_311_agg_tract['agg_graffiti'] = df_DC_311_agg_tract.loc[:, c8].sum(axis=1)

df_DC_311 = pd.concat([df_DC_311_agg_tract[['tract2010']], df_DC_311_agg_tract.loc[:, 'agg_ALL':]], axis=1)

# %%
filename_DC_SLD_agg_tract = os.path.join(dirname, 'machine_learning/ml_DC_SLD_agg_tract.csv')
df_DC_SLD_agg_tract = pd.read_csv(filename_DC_SLD_agg_tract, dtype={'tract_id': str})
# %%
cols_sld = ['tract_id', 'P_WRKAGE',
            ...
            'D4b025', 'D4b050', 'D4d']
df_DC_SLD_agg_tract = df_DC_SLD_agg_tract[cols_sld]
# %%
filename_DC_SVI = os.path.join(dirname, 'machine_learning/ml_DC_SVI.csv')
df_DC_SVI = pd.read_csv(filename_DC_SVI, dtype={'FIPS': str})
# %%
cols_svi = ['FIPS'] + [c for c in df_DC_SVI.columns if c.startswith('EP_')]
df_DC_SVI = df_DC_SVI[cols_svi]
# %%
filename_DC_CDC = os.path.join(dirname, 'machine_learning/ml_DC_CDC.csv')
df_DC_CDC = pd.read_csv(filename_DC_CDC, dtype={'TractFIPS': str})
# %%
...
# %%
# merge data
d1 = pd.merge(df_DC_CDC[['TractFIPS', 'Population2010'] + health_vars], df_DC_311, left_on='TractFIPS',
              right_on='tract2010', how='left').drop(columns='tract2010')
d2 = pd.merge(d1, df_DC_SLD_agg_tract, left_on='TractFIPS', right_on='tract_id', how='left').drop(
    columns='tract_id')
d3 = pd.merge(d2, df_DC_SVI, left_on='TractFIPS', right_on='FIPS', how='left').drop(columns='FIPS')
df_DC_all_data = d3
# %%
# adjust the 311 counts by population
df_DC_all_data_adj = df_DC_all_data.copy()
cols_311_types = df_DC_311.columns.to_list()
cols_311_types.remove('tract2010')
df_DC_all_data_adj[cols_311_types] = df_DC_all_data_adj[cols_311_types].div(
    df_DC_all_data_adj['Population2010'], axis=0)
# %%
# check null values
cols_nan = df_DC_all_data_adj.columns[(df_DC_all_data_adj.isnull().sum() != 0)].to_list()
df_DC_all_data_adj[cols_nan].isnull().sum()
# %%
df_DC_all_data_adj.dropna(subset=cols_nan, inplace=True)
# %%
df_DC_tracts = df_DC_all_data_adj[['TractFIPS']].copy()
df_DC_pop = df_DC_all_data_adj[['Population2010']].copy()
df_DC_all_data_adj.drop(columns=['Population2010', 'TractFIPS'], inplace=True)

# %%
# shorten column names (<= 10 characters)
col_name_map = {'agg_ALL': 'SR_ALL', 'agg_collection': 'SR_COLLECT', 'agg_parking': 'SR_PRKNG', 'agg_signs': 'SR_SIGN',
                'agg_street': 'SR_STREET', 'agg_sanitation': 'SR_SANIT', 'agg_abandoned_vehicle': 'SR_ABANVEH',
                'agg_tree': 'SR_TREE', 'agg_graffiti': 'SR_GRAFFIT',
...
df_DC_all_data_adj.rename(columns=col_name_map, inplace=True)
...
# %%
train_set, test_set = train_test_split(df_DC_all_data_adj, test_size=0.2, random_state=42)
...
df_DC_map_actual = pd.concat([df_DC_tracts, df_DC_pop, df_DC_all_data_adj], axis=1).loc[my_indices, :]
df_DC_map_actual['tract'] = 's' + df_DC_map_actual['TractFIPS']
df_DC_map_actual.reset_index(drop=True, inplace=True)
df_DC_map = pd.concat([df_DC_map_actual, my_df_preds], axis=1)
df_DC_map.rename(columns={'Population2010': 'POP2010'}, inplace=True)
# %%
...
for index, feature in enumerate(my_features):
    if feature in ['P_LOWWAGEr', 'P_LOWWAGEe', 'HHWRKJOBEQ', 'D_X_EXCLAO']:
        df_DC_map['m_' + feature[2:]] = na_mean[index]
        df_DC_map['s_' + feature[2:]] = na_sd[index]
    elif feature == 'WRKSPERJOB':
        df_DC_map['m' + 'WKSPERJOB'] = na_mean[index]
        df_DC_map['s' + 'WKSPERJOB'] = na_sd[index]
    else:
        df_DC_map['m' + feature] = na_mean[index]
        df_DC_map['s' + feature] = na_sd[index]
# %%
# save_csv(df_DC_map, 'DC_map_web')
# %%
...
save_csv(df_lasso_coef, "DC_lasso_coef_stroke")
