...
# %%
filename_Boston_311_agg_tract = os.path.join(dirname, 'machine_learning/ml_Boston_311_agg_tract.csv')
df_Boston_311_agg_tract = pd.read_csv(filename_Boston_311_agg_tract, dtype={'tract2010': str})
# %%
ser_count311 = df_Boston_311_agg_tract.drop(columns=['tract2010']).sum()
...
# %%
# feature engineering
c1 = ['Parking Enforcement']
c2 = ['Requests for Street Cleaning',
      'Request for Pothole Repair',
      'Unshoveled Sidewalk',
      'Street Light Outages',
      'Sidewalk Repair (Make Safe)']
c3 = ['Improper Storage of Trash (Barrels)',
...
c5 = ['Tree Maintenance Requests',
      'Ground Maintenance']
c6 = ['Poor Conditions of Property',
      'Unsatisfactory Living Conditions',
      'Rodent Activity']
c7 = ['Abandoned Vehicles']
c8 = ['Pick up Dead Animal',
      'Animal Generic Request']
c9 = ['Graffiti Removal']
c10 = ['Request for Snow Plowing']

df_Boston_311_agg_tract['agg_ALL'] = df_Boston_311_agg_tract.loc[:, all_311_types].sum(axis=1)
df_Boston_311_agg_tract['agg_parking'] = df_Boston_311_agg_tract.loc[:, c1].sum(axis=1)
df_Boston_311_agg_tract['agg_street'] = df_Boston_311_agg_tract.loc[:, c2].sum(axis=1)
df_Boston_311_agg_tract['agg_trash'] = df_Boston_311_agg_tract.loc[:, c3].sum(axis=1)
df_Boston_311_agg_tract['agg_sign'] = df_Boston_311_agg_tract.loc[:, c4].sum(axis=1)
df_Boston_311_agg_tract['agg_tree'] = df_Boston_311_agg_tract.loc[:, c5].sum(axis=1)
df_Boston_311_agg_tract['agg_poor_condition'] = df_Boston_311_agg_tract.loc[:, c6].sum(axis=1)
df_Boston_311_agg_tract['agg_abandoned_vehicle'] = df_Boston_311_agg_tract.loc[:, c7].sum(axis=1)
df_Boston_311_agg_tract['agg_animal'] = df_Boston_311_agg_tract.loc[:, c8].sum(axis=1)
df_Boston_311_agg_tract['agg_graffiti'] = df_Boston_311_agg_tract.loc[:, c9].sum(axis=1)
df_Boston_311_agg_tract['agg_snow_plowing'] = df_Boston_311_agg_tract.loc[:, c10].sum(axis=1)

df_Boston_311 = pd.concat([df_Boston_311_agg_tract[['tract2010']], df_Boston_311_agg_tract.loc[:, 'agg_ALL':]], axis=1)

# %%
filename_Boston_SLD_agg_tract = os.path.join(dirname, 'machine_learning/ml_Boston_SLD_agg_tract.csv')
df_Boston_SLD_agg_tract = pd.read_csv(filename_Boston_SLD_agg_tract, dtype={'tract_id': str})
# %%
cols_sld = ['tract_id', 'P_WRKAGE',
            ...
            'D4b025', 'D4b050', 'D4d']
df_Boston_SLD_agg_tract = df_Boston_SLD_agg_tract[cols_sld]
# %%
filename_Boston_SVI = os.path.join(dirname, 'machine_learning/ml_Boston_SVI.csv')
df_Boston_SVI = pd.read_csv(filename_Boston_SVI, dtype={'FIPS': str})
# %%
cols_svi = ['FIPS'] + [c for c in df_Boston_SVI.columns if c.startswith('EP_')]
df_Boston_SVI = df_Boston_SVI[cols_svi]
# %%
filename_Boston_CDC = os.path.join(dirname, 'machine_learning/ml_Boston_CDC.csv')
df_Boston_CDC = pd.read_csv(filename_Boston_CDC, dtype={'TractFIPS': str})
# %%
...
# %%
# merge data
d1 = pd.merge(df_Boston_CDC[['TractFIPS', 'Population2010'] + health_vars], df_Boston_311, left_on='TractFIPS',
              right_on='tract2010', how='left').drop(columns='tract2010')
d2 = pd.merge(d1, df_Boston_SLD_agg_tract, left_on='TractFIPS', right_on='tract_id', how='left').drop(
    columns='tract_id')
d3 = pd.merge(d2, df_Boston_SVI, left_on='TractFIPS', right_on='FIPS', how='left').drop(columns='FIPS')
df_Boston_all_data = d3
# %%
# adjust the 311 counts by population
df_Boston_all_data_adj = df_Boston_all_data.copy()
cols_311_types = df_Boston_311.columns.to_list()
cols_311_types.remove('tract2010')
df_Boston_all_data_adj[cols_311_types] = df_Boston_all_data_adj[cols_311_types].div(
    df_Boston_all_data_adj['Population2010'], axis=0)
# %%
# check null values
cols_nan = df_Boston_all_data_adj.columns[(df_Boston_all_data_adj.isnull().sum() != 0)].to_list()
df_Boston_all_data_adj[cols_nan].isnull().sum()
# %%
df_Boston_all_data_adj.drop(columns=['E_PCTLOWWA'], inplace=True)
# %%
cols_nan.remove('E_PCTLOWWA')
df_Boston_all_data_adj.dropna(subset=cols_nan, inplace=True)
# %%
df_Boston_tracts = df_Boston_all_data_adj[['TractFIPS']].copy()
df_Boston_pop = df_Boston_all_data_adj[['Population2010']].copy()
df_Boston_all_data_adj.drop(columns=['Population2010', 'TractFIPS'], inplace=True)

# %%
# shorten column names (<= 10 characters)
col_name_map = {'agg_ALL': 'SR_ALL', 'agg_parking': 'SR_PRKNG', 'agg_street': 'SR_STREET', 'agg_trash': 'SR_TRASH',
                'agg_sign': 'SR_SIGN', 'agg_tree': 'SR_TREE', 'agg_poor_condition': 'SR_POORCON',
                'agg_abandoned_vehicle': 'SR_ABANVEH', 'agg_animal': 'SR_ANIMAL', 'agg_graffiti': 'SR_GRAFFIT',
                'agg_snow_plowing': 'SR_SNOWPLW',
...
df_Boston_all_data_adj.rename(columns=col_name_map, inplace=True)
...
# %%
train_set, test_set = train_test_split(df_Boston_all_data_adj, test_size=0.2, random_state=42)
...
df_Boston_map_actual = pd.concat([df_Boston_tracts, df_Boston_pop, df_Boston_all_data_adj], axis=1).loc[my_indices, :]
df_Boston_map_actual['tract'] = 's' + df_Boston_map_actual['TractFIPS']
df_Boston_map_actual.reset_index(drop=True, inplace=True)
df_Boston_map = pd.concat([df_Boston_map_actual, my_df_preds], axis=1)
df_Boston_map.rename(columns={'Population2010': 'POP2010'}, inplace=True)
# %%
...
for index, feature in enumerate(my_features):
    if feature in ['P_LOWWAGEr', 'P_LOWWAGEe', 'HHWRKJOBEQ', 'D_X_EXCLAO']:
        df_Boston_map['m_' + feature[2:]] = na_mean[index]
        df_Boston_map['s_' + feature[2:]] = na_sd[index]
    elif feature == 'WRKSPERJOB':
        df_Boston_map['m' + 'WKSPERJOB'] = na_mean[index]
        df_Boston_map['s' + 'WKSPERJOB'] = na_sd[index]
    else:
        df_Boston_map['m' + feature] = na_mean[index]
        df_Boston_map['s' + feature] = na_sd[index]
# %%
# save_csv(df_Boston_map, 'Boston_map_web')
# %%
...
save_csv(df_lasso_coef, "Boston_lasso_coef_stroke")
