...
# %%
filename_SF_311_agg_tract = os.path.join(dirname, 'machine_learning/ml_SF_311_agg_tract.csv')
df_SF_311_agg_tract = pd.read_csv(filename_SF_311_agg_tract, dtype={'tract2010': str})
# %%
ser_count311 = df_SF_311_agg_tract.drop(columns=['tract2010']).sum()
...
# %%
# feature engineering
c1 = ['Street and Sidewalk Cleaning']
c2 = ['Graffiti']
c3 = ['Encampments']
c4 = ['Parking Enforcement']
c5 = ['Abandoned Vehicle']
c6 = ['General Request - PUBLIC WORKS']
c7 = ['MUNI Feedback']

df_SF_311_agg_tract['agg_ALL'] = df_SF_311_agg_tract.loc[:, all_311_types].sum(axis=1)
df_SF_311_agg_tract['agg_street_cleaning'] = df_SF_311_agg_tract.loc[:, c1].sum(axis=1)
df_SF_311_agg_tract['agg_graffiti'] = df_SF_311_agg_tract.loc[:, c2].sum(axis=1)
df_SF_311_agg_tract['agg_encampments'] = df_SF_311_agg_tract.loc[:, c3].sum(axis=1)
df_SF_311_agg_tract['agg_parking'] = df_SF_311_agg_tract.loc[:, c4].sum(axis=1)
df_SF_311_agg_tract['agg_abandoned_vehicle'] = df_SF_311_agg_tract.loc[:, c5].sum(axis=1)
df_SF_311_agg_tract['agg_public_works'] = df_SF_311_agg_tract.loc[:, c6].sum(axis=1)
df_SF_311_agg_tract['agg_muni_feedback'] = df_SF_311_agg_tract.loc[:, c7].sum(axis=1)

df_SF_311 = pd.concat([df_SF_311_agg_tract[['tract2010']], df_SF_311_agg_tract.loc[:, 'agg_ALL':]], axis=1)

# %%
filename_SF_SLD_agg_tract = os.path.join(dirname, 'machine_learning/ml_SF_SLD_agg_tract.csv')
df_SF_SLD_agg_tract = pd.read_csv(filename_SF_SLD_agg_tract, dtype={'tract_id': str})
# %%
cols_sld = ['tract_id', 'P_WRKAGE',
            ...
            'D4b025', 'D4b050', 'D4d']
df_SF_SLD_agg_tract = df_SF_SLD_agg_tract[cols_sld]
# %%
filename_SF_SVI = os.path.join(dirname, 'machine_learning/ml_SF_SVI.csv')
df_SF_SVI = pd.read_csv(filename_SF_SVI, dtype={'FIPS': str})
# %%
cols_svi = ['FIPS'] + [c for c in df_SF_SVI.columns if c.startswith('EP_')]
df_SF_SVI = df_SF_SVI[cols_svi]
# %%
filename_SF_CDC = os.path.join(dirname, 'machine_learning/ml_SF_CDC.csv')
df_SF_CDC = pd.read_csv(filename_SF_CDC, dtype={'TractFIPS': str})
# %%
...
# %%
# merge data
d1 = pd.merge(df_SF_CDC[['TractFIPS', 'Population2010'] + health_vars], df_SF_311, left_on='TractFIPS',
              right_on='tract2010', how='left').drop(columns='tract2010')
d2 = pd.merge(d1, df_SF_SLD_agg_tract, left_on='TractFIPS', right_on='tract_id', how='left').drop(
    columns='tract_id')
d3 = pd.merge(d2, df_SF_SVI, left_on='TractFIPS', right_on='FIPS', how='left').drop(columns='FIPS')
df_SF_all_data = d3
# %%
# adjust the 311 counts by population
df_SF_all_data_adj = df_SF_all_data.copy()
cols_311_types = df_SF_311.columns.to_list()
cols_311_types.remove('tract2010')
df_SF_all_data_adj[cols_311_types] = df_SF_all_data_adj[cols_311_types].div(
    df_SF_all_data_adj['Population2010'], axis=0)
# %%
# check null values
cols_nan = df_SF_all_data_adj.columns[(df_SF_all_data_adj.isnull().sum() != 0)].to_list()
df_SF_all_data_adj[cols_nan].isnull().sum()
# %%
df_SF_all_data_adj.dropna(subset=cols_nan, inplace=True)
# %%
df_SF_tracts = df_SF_all_data_adj[['TractFIPS']].copy()
df_SF_pop = df_SF_all_data_adj[['Population2010']].copy()
df_SF_all_data_adj.drop(columns=['Population2010', 'TractFIPS'], inplace=True)

# %%
# shorten column names (<= 10 characters)
col_name_map = {'agg_ALL': 'SR_ALL', 'agg_street_cleaning': 'SR_STCLEAN', 'agg_graffiti': 'SR_GRAFFIT',
                'agg_encampments': 'SR_ENCAMP', 'agg_parking': 'SR_PRKNG', 'agg_abandoned_vehicle': 'SR_ABANVEH',
                'agg_public_works': 'SR_PUBWK', 'agg_muni_feedback': 'SR_MUNIFBK',
...
df_SF_all_data_adj.rename(columns=col_name_map, inplace=True)
...
# %%
train_set, test_set = train_test_split(df_SF_all_data_adj, test_size=0.2, random_state=42)
...
df_SF_map_actual = pd.concat([df_SF_tracts, df_SF_pop, df_SF_all_data_adj], axis=1).loc[my_indices, :]
df_SF_map_actual['tract'] = 's' + df_SF_map_actual['TractFIPS']
df_SF_map_actual.reset_index(drop=True, inplace=True)
df_SF_map = pd.concat([df_SF_map_actual, my_df_preds], axis=1)
df_SF_map.rename(columns={'Population2010': 'POP2010'}, inplace=True)
# %%
...
for index, feature in enumerate(my_features):
    if feature in ['P_LOWWAGEr', 'P_LOWWAGEe', 'HHWRKJOBEQ', 'D_X_EXCLAO']:
        df_SF_map['m_' + feature[2:]] = na_mean[index]
        df_SF_map['s_' + feature[2:]] = na_sd[index]
    elif feature == 'WRKSPERJOB':
        df_SF_map['m' + 'WKSPERJOB'] = na_mean[index]
        df_SF_map['s' + 'WKSPERJOB'] = na_sd[index]
    else:
        df_SF_map['m' + feature] = na_mean[index]
        df_SF_map['s' + feature] = na_sd[index]
# %%
# save_csv(df_SF_map, 'SF_map_web')
# %%
...
save_csv(df_lasso_coef, "SF_lasso_coef_stroke")
